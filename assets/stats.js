(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const groups = ["mock01", "mock02", "finalbank"].map((exam) => { const values = records.filter((item) => item.exam === exam); return { exam, label: exam === "mock01" ? "모의고사 1회" : exam === "mock02" ? "모의고사 2회" : "기말 CBT", attempts: values.length, best: values.length ? Math.max(...values.map((item) => item.score)) : 0 }; });
